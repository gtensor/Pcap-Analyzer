...
from pcapalyzer import app
from pcapalyzer.forms import Upload, ProtoFilter
from pcapalyzer.utils.data_extract import (
    web_data, telnet_ftp_data, mail_data, sen_data, client_info)
from pcapalyzer.utils.except_info import exception_warning
from pcapalyzer.utils.file_extract import web_file, ftp_file, mail_file, all_files
from pcapalyzer.utils.flow_analyzer import (
    time_flow, data_flow, get_host_ip, data_in_out_ip,
    proto_flow, most_flow_statistic)
from pcapalyzer.utils.ipmap_tools import getmyip, get_ipmap, get_geo
from pcapalyzer.utils.pcap_decode import PcapDecode
from pcapalyzer.utils.pcap_filter import get_all_pcap, proto_filter, showdata_from_id
from pcapalyzer.utils.proto_analyzer import (
    common_proto_statistic,
    pcap_len_statistic, http_statistic, dns_statistic,
    most_proto_statistic)
from pcapalyzer.utils.upload_tools import allowed_file, get_filetype, random_name
...
def protoanalyzer():
    if not PCAPS:
        ...
        return render_template('./dataanalyzer/protoanalyzer.html', data=list(data_dict.values()),
                               pcap_len=pcap_len_dict, pcap_keys=list(pcap_count_dict.keys()), http_key=http_key_list,
...
def flowanalyzer():
    if not PCAPS:
        ...
        return render_template('./dataanalyzer/flowanalyzer.html', time_flow_keys=list(time_flow_dict.keys()),
                               ...
                               ip_flow=data_ip_dict, proto_flow=list(proto_flow_dict.values()),
                               most_flow_key=most_flow_key, most_flow_dict=most_flow_dict)
...
def ipmap():
    if not PCAPS:
        ...
        if myip:
            ...
            print(ip_value_list)
            print(geo_dict)
            return render_template('./dataanalyzer/ipmap.html', geo_data=geo_dict, ip_value=ip_value_list,
                                   mygeo=myip_geo)
        else:
